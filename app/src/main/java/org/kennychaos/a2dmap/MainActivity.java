...
public class MainActivity extends AppCompatActivity implements TCPListener {
    ...
    private void first(View view) {
        String _ = Base64Util.encode(("0").getBytes());
    ...
    public void receive(byte[] bytes, int length) {
        logUtil.show("receive = " + Arrays.toString(bytes) +" source = " + bytesToHexString(bytes) + " length = " + length,LogUtil.LOG_LEVEL_INFO);
    }

    @Override
    public void sent(byte[] bytes, int length) {
        logUtil.show("sent = " + Arrays.toString(bytes) +" source = " + bytesToHexString(bytes) + " length = " + length,LogUtil.LOG_LEVEL_INFO);
    }
